...
impl HeightMap {
    ...
    pub fn get_shortest_path_len(&self, from: &Position, to: &Position) -> u16 {
        ...
        while !queue.is_empty() {
            ...
            let current_height = self.get_height(&current.position);

            for neighbour in self
                .get_neighbours(&current.position)
                .iter()
                .filter(|neighbour| self.get_height(neighbour) <= current_height + 1)
            {
    ...
    pub fn get_shortest_path_len_start_to_end(&self) -> u16 {
        self.get_shortest_path_len(&self.start, &self.end)
    }
}
